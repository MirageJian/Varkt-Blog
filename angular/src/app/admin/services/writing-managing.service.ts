...
export class WritingManagingService extends ArticleService {
  ...
  deleteArticle(id: number): Observable<any> {
    ...
    return this.http.delete('/api/article', {params: params}).pipe(catchError(this.handleError));
  }
  deleteComment(id: number) {
    let params = new HttpParams();
    params = params.set('id', id.toString());
    return this.http.delete('/api/comment_managing', {params: params}).pipe(catchError(this.handleError));
  }
  addOrUpdateArticle(article: ArticleModel) {
    if (article.id == 0)
      return this.http.post('/api/article', prePolish(article)).pipe(catchError(this.handleError));
    else
      return this.http.put('/api/article', prePolish(article)).pipe(catchError(this.handleError));
  }
}
